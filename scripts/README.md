...
- **`tools/`** – Python scripts. Run from project root or `scripts/tools/` (for `data_loader` imports).
  ...
  - **`bbox_editor.py`** – Interactive bbox editor. Output should be `scripts/data/hotspots/<scene_id>.json`.
  ...
  - **`generate_scene_audio.py`** – One scene, all or one lang: `python3 generate_scene_audio.py --scene <id> [--lang xx]`.
  - **`regenerate_hi_missing.py`** – Regenerate Hindi voice files (ElevenLabs) for all scenes.
  - **`migrate_audio_to_scene_folders.py`** – One-off; already run. Copies flat voice files into `voices/<lang>/<scene_id>/`.
...
2. Run `python3 scripts/tools/bbox_editor.py --image assets/images/scenes/scene_<id>.<ext> --output scripts/data/hotspots/<id>.json` and save bboxes.
...
4. Run `python3 scripts/tools/dump_data.py` to update `data/` and `game_data.json`.
5. Generate audio: `python3 scripts/tools/generate_scene_audio.py --scene <id>` (or scene-specific script if added).
