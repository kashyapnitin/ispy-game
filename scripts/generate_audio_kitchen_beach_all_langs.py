...
def load_hotspot_objects(filename: str) -> list[str]:
    hotspots_path = ROOT / "scripts" / filename
...
SCENES = {
    "kitchen": "kitchen_hotspots.json",
    "beach": "beach_hotspots.json",
    "playground": "playground_hotspots.json",
}
...
def main() -> None:
    ...
    for scene_id, hotspot_file in SCENES.items():
        obj_names = load_hotspot_objects(hotspot_file)
        print(f"{scene_id} hotspot objects: {len(obj_names)}")

        scene = data["scenes"].get(scene_id, {})
        scene_langs = sorted(scene.get("i18n", {}).keys())
        if not scene_langs:
            raise SystemExit(f"No i18n languages found for {scene_id} in game_data.json.")
        ...
        for lang in scene_langs:
            ...
            for obj_name in obj_names:
                ...
                for kind, text in (("hint", hint_text), ("found", found_text)):
                    out_path = lang_dir / f"{kind}_{safe}.mp3"
