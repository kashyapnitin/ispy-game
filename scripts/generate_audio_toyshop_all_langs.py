...
def load_toyshop_objects() -> list[str]:
    hotspots_path = ROOT / "scripts" / "toyshop_hotspots.json"
...
def main() -> None:
    ...
    obj_names = load_toyshop_objects()

    print(f"Toyshop hotspot objects: {len(obj_names)}")

    scene = data["scenes"].get("toyshop", {})
    scene_langs = sorted(scene.get("i18n", {}).keys())
    if not scene_langs:
        raise SystemExit("No i18n languages found for toyshop in game_data.json.")
